...
export default function TGAT2ResultPage() {
  ...
  if (displayQuestions.length === 0 && latestResult) {
    if (latestResult.questionIds) {
      displayQuestions = latestResult.questionIds
        .map(id => TGAT2_QUESTIONS.find(q => q.id === id))
        .filter(Boolean) as typeof TGAT2_QUESTIONS;
    } else {
      displayQuestions = TGAT2_QUESTIONS.slice(0, latestResult.total);
    }
    ...
    displayTimeRemaining = latestResult.mode === 'exam' ? 4500 - latestResult.timeSpent : 0;
  }
  ...
  // Time Analysis
  const totalTimeSpent = displayQuestions.length > 0 ? (displayExamMode === 'exam' ? 4500 - displayTimeRemaining : Object.values(displayPerQuestionTime).reduce((a,b)=>a+b, 0)) : 0;
      ...
      <div className="max-w-5xl mx-auto space-y-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1">
            <ScoreChart score={displayScore} total={displayQuestions.length} label="คะแนน TGAT2" colorClass="from-neon-purple to-purple-500" />
          </div>
