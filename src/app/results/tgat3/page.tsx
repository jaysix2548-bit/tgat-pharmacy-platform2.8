  ...
  if (displayQuestions.length === 0 && latestResult) {
    if (latestResult.questionIds) {
      displayQuestions = latestResult.questionIds
        .map(id => TGAT3_QUESTIONS.find(q => q.id === id))
        .filter(Boolean) as typeof TGAT3_QUESTIONS;
    } else {
      displayQuestions = TGAT3_QUESTIONS.slice(0, latestResult.total);
    }
      ...
      <div className="max-w-5xl mx-auto space-y-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1">
            <ScoreChart score={displayScore} total={displayQuestions.length} label="คะแนน TGAT3" colorClass="from-orange-400 to-yellow-500" />
          </div>
